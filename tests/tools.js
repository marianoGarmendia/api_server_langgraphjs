...
const normalize = (text) => text
    ...
    .trim();
// Función de búsqueda
function buscarProductos(query, categoria = "todas") {
    ...
    if (categoria === "todas" || categoria === "palas") {
        productos.push(...KATALOGO_PRODUCTOS_KOMBAT.palas.map((p) => ({ ...p, categoria: "palas" })));
    }
    if (categoria === "todas" || categoria === "accesorios") {
        productos.push(...KATALOGO_PRODUCTOS_KOMBAT.accesorios.map((p) => ({ ...p, categoria: "accesorios" })));
    }
    if (categoria === "todas" || categoria === "indumentaria") {
        productos.push(...KATALOGO_PRODUCTOS_KOMBAT.indumentaria.map((p) => ({ ...p, categoria: "indumentaria" })));
    }
    ...
    const resultados = productos
        ...
        for (const term of queryTerms) {
            if (nombreNorm.includes(term))
                relevancia += 2;
            if (keywordsNorm.some((kw) => kw.includes(term)))
                relevancia += 1;
        }
...
// Tool definition
export const linkProductoTool = tool(async ({ query, categoria = "todas" }) => {
    ...
    description: "Busca el link directo a un producto específico de KOMBAT (palas, accesorios, indumentaria). Usar cuando el cliente pregunta por un producto específico y se necesita el link para comprarlo o verlo.",
    schema: z.object({
        query: z
            .string()
...
`;
export const tiendaKombatTool = tool(async ({ query }) => {
...
export const priceTool = tool(async ({ query }) => {
    const model = new ChatOpenAI({
        model: "gpt-5-mini",
        apiKey: process.env.OPENAI_API_KEY,
    });
...
`;
    ...
    description: "Obtiene la información relacionada sobre precios, cuotas, promociones vigentes",
    schema: z.object({
        query: z
            .string()
            .describe("La consulta del cliente relacionada sobre precios, cuotas, promociones vigentes, lo mas detallada y estructurada posible para que el modelo pueda responder de la mejor manera posible, debe ser como un mensaje del usuario"),
    }),
});
export const infoCatalogoVulcano = tool(async ({ query }) => {
    ...
    const includes = (value) => q.includes(normalize(value));
    const shape = (q.includes("lagrima") && "Lágrima") ||
        (q.includes("diamante") && "Diamante") ||
        (q.includes("redonda") && "Redonda") ||
        undefined;
    const hardness = (q.includes("blanda") && "Blanda") ||
        (q.includes("media") && "Media") ||
        (q.includes("dura") && "Dura") ||
        undefined;
    const balance = (q.includes("alto") && q.includes("balance") && "Alto") ||
        (q.includes("medio") && q.includes("balance") && "Medio") ||
        (q.includes("bajo") && q.includes("balance") && "Bajo") ||
        undefined;
    const potencia = q.includes("potencia") && q.includes("alto")
        ? "Alto"
        : q.includes("potencia") && q.includes("medio")
            ...
            : q.includes("potencia") && q.includes("bajo")
                ...
                : q.includes("potencia")
                    ? "Alto"
                    : undefined;
    const control = q.includes("control") && q.includes("alto")
        ? "Alto"
        : q.includes("control") && q.includes("medio")
            ...
            : q.includes("control") && q.includes("bajo")
                ...
                : q.includes("control")
                    ? "Alto"
                    : undefined;
    const allPalas = CATALOGO_VULCANO.lineas.flatMap((l) => l.palas.map((p) => ({ ...p, linea: l.nombre_linea })));
    const exactName = allPalas.find((p) => includes(p.tipo_de_pala));
    const matches = allPalas.filter((p) => {
        if (shape && normalize(p.forma) !== normalize(shape))
            return false;
        if (hardness && normalize(p.dureza) !== normalize(hardness))
            return false;
        if (balance && normalize(p.balance) !== normalize(balance))
            return false;
        if (potencia && normalize(p.potencia) !== normalize(potencia))
            return false;
        if (control && normalize(p.control) !== normalize(control))
            return false;
        return true;
    });
    const ranked = (exactName ? [exactName] : matches.length ? matches : allPalas)
        .map((p) => {
    ...
    const body = ranked
        .map((p) => `- ${p.tipo_de_pala} (${p.linea}) | forma: ${p.forma}, dureza: ${p.dureza}, balance: ${p.balance}, potencia: ${p.potencia}, control: ${p.control}, nucleo: ${p.nucleo}, material: ${p.material}, peso: ${p.peso}`)
        .join("\n");
    ...
    description: "Obtiene la información relacionada sobre el catálogo de palas Vulcano",
    schema: z.object({
        query: z
            .string()
            .describe("La consulta del cliente relacionada sobre el catálogo de palas Vulcano, o tambien una query para buscar en el catálogo sin que el usuario haya preguntado específicamente sobre el catálogo, trata de ser lo más específico posible"),
    }),
});
...
export const infoPalasKombat = tool(async (input, config) => {
    ...
    try {
        ...
        if (title)
            and.push({ title: { $eq: title } });
        // if (input.brands?.length) and.push({ brand: { $in: input.brands } });
        const results = await vectorStore.similaritySearchWithScore(input.query, 6, { preFilter: { $and: and } });
    ...
    description: "Busca contexto relevante para responder preguntas sobre como elegir palas kombat.",
    schema: SearchInput,
});
