...
export type ProductoKombat = {
  nombre: string;
  keywords: string[];
  url: string;
  categoria: "palas" | "accesorios" | "indumentaria";
};
...
export const routerSchemaV2 = z.object({
  area: AreaEnum.describe(
    `Área a la cual derivar según la intención del usuario:
  ...
  confianza: ConfianzaEnum.describe(
    `Nivel de certeza en la clasificación:
  ...
  .max(100)
  .describe(
    "Descripción breve y concisa de lo que el usuario quiere o necesita (máx 100 caracteres)"
  ),
  
  requiere_herramienta: z
  .boolean()
  .describe(
    "true si el área necesita consultar una herramienta para responder correctamente, false si se puede responder con información fija"
  ),
  
  herramienta_sugerida: HerramientaEnum.nullable().describe(
    `Herramienta a utilizar según el área:
  ...
  .describe(
    `Acción recomendada para el siguiente nodo:
  ...
  .nullable()
  .describe(
    `Canal al que derivar si aplica:
...
export type RouterOutput = z.infer<typeof routerSchemaV2>;
...
export const faqSchema = z.object({
  ...
  .boolean()
  .describe(
    "true si la consulta puede responderse con una FAQ existente, false si requiere procesamiento adicional"
  ),
  
  confianza: z
  .enum(["alta", "media", "baja"])
  .describe(
    "Nivel de certeza de que la FAQ responde correctamente la consulta"
  ),
  ...
  .nullable()
  .describe(
    "Pregunta frecuente que matchea con la consulta del usuario, o null si no hay match"
  ),
  ...
  .nullable()
  .describe(
    "Respuesta consolidada y adaptada al tono de WhatsApp (cálida, breve, con links). Null si isFaq es false"
  ),
  
  requiere_tool: z
  .boolean()
  .describe(
    "true si aunque sea FAQ, necesita complementar con info de herramientas (ej: precios actualizados)"
  ),
});
...
export const summarizeSchema = z.object({
  cliente: z
    .string()
    .nullable()
    .describe("Nombre del cliente si lo mencionó en la conversación, null si no lo sabemos"),
    resumen: z
      .string()
      .describe("Resumen conciso de la conversación (máximo 2-3 oraciones)"),
  
  calificacion: z
    .enum(["SinInteres", "BajoInteres", "Interesado", "MuyInteresado"])
    .describe("Calificación del lead según su intención de compra"),
  
  
  productos_mencionados: z
    .array(z.string())
    .describe("Lista de productos específicos que se mencionaron en la conversación"),
  
  siguiente_accion: z
    .string()
    .nullable()
    .describe("Acción de seguimiento recomendada si aplica (ej: 'enviar catálogo', 'seguimiento en 48hs')"),
  
  motivo_calificacion: z
    .string()
    .describe("Breve justificación de por qué se asignó esa calificación"),
});

export type SummarizeOutput = z.infer<typeof summarizeSchema>;
export type FaqOutput = z.infer<typeof faqSchema>;
export type RouterOutputSimple = z.infer<typeof routerSchemaSimple>;
export type LinkProductoInput = z.infer<typeof linkProductoSchema>;
export type LinkProductoOutput = z.infer<typeof linkProductoOutputSchema>;
